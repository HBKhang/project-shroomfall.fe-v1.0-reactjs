...
export function ItemPlaceableConfig({
    config,
    onChange
}: Props) {
    ...
    return (
        <div className="flex flex-col gap-4">
            <h4 className="text-sm font-semibold text-sky-900 tracking-tight">Placeable Object Settings</h4>
            <div className="w-full">
                <label className={labelClass}>Spawned World Entity ID</label>
                <input
                    type="text"
                    value={config?.entityDefinitionID ?? ''}
                    onChange={(e) => onChange("entityDefinitionID", e.target.value)}
                    className={`${baseInputClass} font-mono`}
                    placeholder="ent_furniture_wooden_chair"
                    required
                />
            </div>
        </div>
